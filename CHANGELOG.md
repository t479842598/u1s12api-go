...
- **后台默认主题改为 U1S1 官网风（`u1s1-official`）**：暖纸米底 + 细方格纸、黑色面板/主按钮、橙红（brand）硬投影、绿/橙/蓝实色徽章、等宽数字——对齐官网控制台观感；侧栏账号卡黑底反色 + 橙红 `5px 5px 0` 硬投影，激活导航项黑底 + 橙红序号徽章；暗色变体为深暖黑 + 米白描边白硬投影 + 橙红保留
- **主题切换器（项目中枢 ThemeSwitcher 同构）**：顶栏下拉选择 8 套主题（U1S1 官网 / 青瓷苔原 / 黑钨 / 新旷野主义 / 苹果磨砂玻璃 / 透明玻璃 / 纸上墨 / 终端），每项带三色预览点与中文说明；自 devhub/freebuff2api-go 同构移植，含材质层变量（半透明/模糊/硬投影由主题驱动）与各主题明暗变体
- **明暗独立切换**：顶栏三态循环按钮（跟随系统 → 亮 → 暗），默认跟随系统并实时响应 `prefers-color-scheme`；主题与明暗分别持久化（`u1s12api.admin.theme.v2` / `u1s12api.admin.mode`），旧 key 自动迁移；`index.html` 防闪烁内联脚本先于首帧恢复

### 变更

- **布局重构为「固定通栏顶栏 + 悬浮卡片侧栏」**：顶栏（logo + 主题/明暗/退出）与左侧栏固定不随内容滚动，仅主内容区滚动；侧栏由贴边通栏改为悬浮卡片组——账号卡（`U1S12API ACCOUNT` + 会话状态点）、导航卡（01–08 序号徽章 + 主/副标题）、官网公告/更新记录下划线文字链（公告数徽章与版本落后提示保留）；移动端抽屉导航沿用并加入官网动态入口

### 验证

- `npm run build`（tsc + vite）与 `go build` 通过；本地起服务浏览器实测：默认主题官网观感、8 主题切换即时生效、明暗三态、刷新后持久化（`data-theme`/`data-mode` 不变）、主区滚动 500px 顶栏/侧栏固定、390px 视口移动端抽屉可用
...
- **指纹同步 u1s1-cli 1.8.1**：`U1S1_VERSION` 默认值 `1.8.0` → **`1.8.1`**（`internal/config` 默认值 / `.env.example` / README / 生产 `.env`）。`x-u1s1-version` 头随之上到 1.8.1

### 逆向核对（1.8.1 vs 1.8.0）

官方 09-05 00:38 UTC 发 1.8.1。逐文件 diff 1.8.0 与 1.8.1 tarball，**请求路径协议零变化**：

- `device-auth.js`、`config.js`、`login.js`、`api.js` **逐字节未变** → DPoP 结构、签名代理、请求头集合、attestation 注入全部不变
- 唯一差异是 `agent-setup.js`（`writeCompactUiExtension` 参数化输出目录 + `TOOL_SUMMARY_LABELS` 去类型标注）与 `.d.ts`——纯客户端 MCP 扩展配置，与网关请求无关
- 结论：只需跟版本号，无代码改动

### 背景（本次 403 client_integrity_review 排查，2026-09-05）

- **13:03（北京）** 生产收到首个 403 `client_integrity_review`：`479842598@qq.com` 被拒，「为了保护你的账号和用量安全，本次请求已暂停。请升级并重新登录 u1s1」——上游已把 1.8.1 视为当前版本，仍报 1.8.0 的请求被新的完整性审查采样拦截（同秒另一条并发请求仍 200，判定为采样而非封号）
- 服务按既定 403 策略自动停用该账号（`enabled=0` + 原因入库），无指纹头/attestation/401 异常，非本地代码问题
- 升级到 1.8.1 后需把被停用的账号重新启用（后台账号管理 → 启用），无账号被上游封禁
