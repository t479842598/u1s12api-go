...
// TestChatHeadersShape 校验 chat 请求指纹头与官方 CLI 一致（逆向自 u1s1-cli 0.19.5）。
func TestChatHeadersShape(t *testing.T) {
	m, err := NewManager(t.TempDir()+"/fp.json", "macos-arm64")
	if err != nil {
		t.Fatal(err)
	}
	h := m.ChatHeaders("u1s1-testkey", "0.19.5")
